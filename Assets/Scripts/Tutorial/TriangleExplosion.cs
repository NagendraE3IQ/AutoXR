...
public class TriangleExplosion : MonoBehaviour
{
    ...
    public void functionExplode()
    {
        StartCoroutine(SplitMesh(true));
        HeadLookManager.Instance.LookCount += 1;

        if(HeadLookManager.Instance.LookCount >= 4)
        {
            StepControllerScript.Instance.CallNextStep();
            HeadLookManager.Instance.HeadPointer.SetActive(false);
            HeadLookManager.Instance.Eye.SetActive(false);
        }
    }
}