...
{    
    ...
    void Update()
    {
        ...
        hitInfo1 = CreateRay(maxdistance, CasterCam.transform);
        if (Teleport.instance)
        {
            if (!Teleport.instance.IsTeleportButtonDown(CasterCam.GetComponent<Hand>()))
            {
                renderline(hitInfo1, maxdistance, CasterCam, line, pointertip);
